...
export class LocaleSwitcher {
    ...
    getCurrentLocale(defaultLocale = "en") {
    ...
    setLocale(locale) {
        if (!this.availableLocales.includes(locale)) {
            console.warn(`[LocaleSwitcher] Locale "${locale}" is not available. Available locales: ${this.availableLocales.join(", ")}`);
            return;
        }
    ...
    createSwitcherHTML(containerId) {
        if (typeof document === "undefined")
            return;
        const container = document.getElementById(containerId);
        if (!container) {
            console.error(`[LocaleSwitcher] Container with ID "${containerId}" not found`);
            return;
        }
        // Create label
...
export const setLocale = (locale) => defaultLocaleSwitcher.setLocale(locale);
export const getLocale = () => defaultLocaleSwitcher.getLocale();
export const getAvailableLocales = () => defaultLocaleSwitcher.getAvailableLocales();
export const createSwitcher = (containerId) => defaultLocaleSwitcher.createSwitcherHTML(containerId);
